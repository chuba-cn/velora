...
const upsertEmail = async(email: EmailMessage, accountId: string, index: number) => {
  ...
  try {
    ...
    if (!fromAddress) {
      console.error(`Failed to upsert from-addresses from email ${email.bodySnippet}`);
      return;
    }
...
const upsertEmailAttachement = async (emailId: string, attachment: EmailAttachment) => { 
  try {
    ...
    console.error(`Failed to upser attachment for email ${emailId}: ${error as any}`)
  }
}