...
export async function generateEmail(context: string, prompt: string) {
  ...
  return { output: stream.value, loadingState: loadingState.value };
};

export async function generateAutoCompletion(input: string) {
  const stream = createStreamableValue("");
  ...
  void (
    async () => {
      ...
      for await (const chunks of textStream) {
