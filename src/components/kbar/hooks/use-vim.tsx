...
const useVim = () => {
  ...
  const { threadId, setThreadId, accountId } = useThreads();

  const [done] = useLocalStorage("velora-done", false);
  ...
  const setUndone = api.account.setUndone.useMutation({
    onMutate: async (payload) => {
      ...
      const previousData =
        queryClient.getQueryData<RouterOutputs["account"]["getThreads"]>(
          queryKey,
        );
